...
2. Fetch boxscore.email/mlb's **current** edition (if WebFetch is blocked, `curl` with a normal browser User-Agent). Also skim the wire desks — **mlb.com/news** and **espn.com/mlb's "Top Headlines"** — for candidate News Around the League items (they never override boxscore.email on scores/stats). Cross-check team affiliations on baseball-reference.com. **Never fabricate**; omit anything unverifiable.
   ...
   - **Awards are decided by the ballot, not the poll.** Never report a Cy Young, MVP, Rookie of the Year, or any other honor as won unless baseball-reference.com's awards pages (`https://www.baseball-reference.com/awards/`) list the winner. A poll or straw vote is reported as a poll, sponsor named; "unanimous" describes the poll, not the award. See `recipe.md`, Sources of truth.
...
5. Build the edition per `schema/edition.schema.json`:
   ...
   - **Postseason** (from the morning after the regular season's last day through the morning after the World Series ends — dates from b-ref's "Upcoming Dates"): set `meta.postseason: true`. Every game not starred as Game of the Day still goes in `rest_of_the_card`, but at **Game-of-the-Day length** with a `subtitle` — the renderer prints it as "The Postseason Card" in one column. Off days keep `hot_stove` + `countdown` (to the next game, per b-ref) but the news is series previews, not offseason matter. If boxscore.email lacks yesterday's postseason games, fall back to b-ref's box scores for the date rather than calling it an off day. See `recipe.md`, "The postseason."
...
| Writing a playoff game as a two-sentence card note, or forgetting `meta.postseason` | October games read like a Tuesday in June; the site keeps its summer layout and "hot stove" wording on off days | Set the flag by b-ref's dates; give every postseason game a headline, subtitle, and a full-length body |
